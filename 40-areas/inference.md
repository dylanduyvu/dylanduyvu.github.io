...
- **Active:** [[inference-model-opportunity-radar|Inference Model Opportunity Radar]] has a live-tested V0 collection and reporting spine. The next project stage is to automate and harden recurring collection so the radar continuously records market state and builds reliable history. The next analysis layer should add checked model-footprint facts and show the demand-versus-size frontier. Quantitative under-service rules and one final model choice come after the evidence is stable.
