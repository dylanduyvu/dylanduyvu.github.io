...
- **Active:** [[inference-model-opportunity-radar|Inference Model Opportunity Radar]] now has a hosted automatic collection spine and a local browser dashboard. The next stage is to observe unattended runs, build provider-history coverage, add checked model-footprint facts, show the demand-versus-size frontier, and host the frontend on Vercel. Quantitative under-service rules and one final model choice come after the evidence is stable.
...
- Observe the first unattended daily and weekly AWS runs, then keep collection active until each mapped model has three endpoint capture days across seven calendar days and two Hugging Face capture days. Host the read-only frontend on Vercel after its AWS export access path is defined. Then define one justified quantitative workload profile and re-run the gates. Treat the earlier GLM-5.2 provider-count difference as unconfirmed until a model-level provider-organization cross-check exists.
