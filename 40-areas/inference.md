...
- **Radar operations:** The public V1C Supply page went live on 2026-08-25. Its first successful hosted supply run completed 74 requests, produced 74 model records, and mapped 72 current routes. The collector now runs four times per UTC day. No model has enough supply history yet, so the page describes current supply but does not identify an under-served model.
- **Hosted radar status:** The automatic AWS collection spine became operational on 2026-08-24. Manual demand and full catalog runs passed before the daily 3:00 PM America/New_York demand timer and Sunday 4:00 PM catalog timer were enabled. Raw evidence and SQLite backups are in private S3 storage; accepted publications are in private Aurora PostgreSQL; and the final current-export hash matched across EC2, S3 history, the S3 pointer, and Aurora. This proves the initial hosted publication path, not long-term schedule reliability or an under-served model.
...
- **Revised model-selection goal:** The practical target is the smallest open-weight model with the strongest durable demand, if one choice can be justified. This should begin as a demand-versus-size frontier, not one universal score. Total parameters, active parameters, weight memory, and the minimum hardware needed at a target context and speed can rank models differently. The current radar has demand and provider evidence but does not yet normalize these model-footprint measures.
...
- **Active:** [[inference-model-opportunity-radar|Inference Model Opportunity Radar]] now has a hosted automatic collection spine and a public demand-and-supply dashboard. The next stage is to verify the first unattended supply run, build seven calendar days of provider history, and add checked model-footprint facts to the demand-versus-size frontier. Quantitative under-service rules and one final model choice come after the evidence is stable.
...
- [[dylan-openrouter-provider-event-remarks-2026-08-20|Dylan: OpenRouter receives about 40 provider applications per day and accepts about one]]: first-hand event remarks on application volume, approval flow, and provider legal review as a current bottleneck.
...
- What does OpenRouter include in provider legal review, and which documents can a new provider prepare before it applies?
...
- Which definition of model size best predicts the minimum cost and hardware needed for the first serving offer?
...
- Observe the first unattended supply run, then keep collection active until each mapped model has three endpoint capture days across seven calendar days and two Hugging Face capture days. Then define one justified quantitative workload profile and re-run the gates. Treat the earlier GLM-5.2 provider-count difference as unconfirmed until a model-level provider-organization cross-check exists.
- Add Vercel as the second hosted-demand collector. Then test AnyRouter, LLM Gateway, and BharatRouter as smaller source-labeled confirmations. Add Requesty and Hugging Face for supply and quality. Do not add raw traffic across routers or treat routed-through-OpenRouter traffic as independent.
- Add checked total-parameter, active-parameter, weight-memory, and minimum-serving-footprint facts where available. Then show the models that have the strongest durable demand for their measured size without calling one model optimal before a workload and hardware test exist.
