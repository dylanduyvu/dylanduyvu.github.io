...
**Verdict:** Scoping decision supported. Two properties do not relax even in the minimal experiment: strict pre-action frame ordering (no future-information leakage) and verified next-action labels. Relaxable: dual-monitor synchronization, cryptographic provenance, joint AX+DOM element identity, exact element identity, sub-second timing precision, perfectly clean trajectories.
...
## TL;DR
- **Your scoping decision is broadly correct and well-supported by the published evidence.** For a first qualitative next-action-prediction (NAP) experiment, a simple dataset (latest active-monitor screenshot, active app/window/URL, click/key/app-switch events, approximate target identity, manually verified next-action labels) is sufficient — the literature shows useful next-action and next-app prediction from far less fidelity than a "complete dataset." The synchronized dual-monitor, cryptographic-provenance, joint AX+DOM dataset is genuinely load-bearing only for a reliable always-on automated product.
- **Two fidelity properties cannot be relaxed even in a minimal first experiment: (1) correct pre-action frame ordering (pairing each action with the last distinct frame *before* it, to avoid future-information leakage), and (2) verified next-action labels.** Both are cheap to get right and both are where published pipelines (OpenCUA/AgentNet, Mind2Web, AITW) invest deliberate engineering. Approximate *element grounding* and single-monitor capture, by contrast, are shown to be relaxable.
...
5. **In-domain next-action prediction saturates with very little data; out-of-domain needs 1–2 orders of magnitude more.** This is the single most decision-relevant scaling result for a personal (in-domain-by-construction) NAP system.
6. **Multi-monitor capture is essentially unrepresented in agent benchmarks** — OSWorld, WindowsAgentArena, and AndroidWorld are all single-screen — which means there is no published evidence that dual-monitor fidelity matters for prediction quality.
...
**What each actually required:** none of these required synchronized dual-monitor capture, cryptographic provenance, or joint AX+DOM identity. Event logs plus sequence models deliver 46–86% next-app accuracy; that is the closest published analogue to your first experiment, and it validates the "simple dataset" scope.

### Sub-question 5: Scaling vs. fidelity trade-off

**Data quantity and diversity substantially compensate for lower per-record fidelity — up to a point.**

- **The decisive result for a personal system (Li et al., "On the Effects of Data Scale on UI Control Agents," NeurIPS 2024, arXiv 2406.03679, Google DeepMind).** Fine-tuning PaLM-2S on the AndroidControl dataset (15,283 demos, 833 apps; a11y-tree text input, *no screenshots*): the best fine-tuned model reaches "71.5% accuracy on high-level instructions and 86.6% on low-level instructions in-domain." Low-level accuracy already beats all non-fine-tuned models with just **5 examples**; high-level needs ~1,000 episodes. Both in-domain curves are log-linear (R²>0.95), extrapolating to ~500K/1M episodes for 95% step accuracy on low/high-level. **Out-of-domain** grows far slower — the paper extrapolates that "10M and 150M episodes would be required" (low-level and high-level respectively) for the same target — and the in-domain-vs-unseen gap widens with model capacity (up to ~12 pp on high-level tasks). Verbatim conclusion: "fine-tuning scales favorably in domain, but out of domain, it requires one or two orders of magnitude more data." **A personal NAP system is in-domain by construction** (your apps, your habits), which is exactly the regime where modest data suffices.
...
- **Multi-monitor capture is unrepresented.** OSWorld defines its observation as a single screenshot (up to 1920×1080) plus optional a11y tree; WindowsAgentArena is built on the same single-VM framework; AndroidWorld is a single emulator screen. There is no published benchmark evidence that dual-monitor synchronization affects prediction quality — because no major benchmark models it. This strongly supports deferring dual-monitor fidelity to the product stage.
...
4. **Multi-monitor sync, exact sub-second timing, cryptographic provenance, no-missing/duplicate guarantees** — defer to the always-on product, where capture reliability (the Recall lesson) and execution grounding genuinely require them.

**Data strategy:** Favor *breadth of situations* over per-record perfection (robotics scaling-law lesson). Because your system is in-domain by construction, expect the favorable AndroidControl in-domain scaling — useful intent prediction from hundreds-to-low-thousands of examples, not millions.
...
**Can relax (published evidence shows these are tolerable or compensable):**
...
- **Dual-monitor synchronization** — unrepresented in all major benchmarks; no evidence it affects prediction quality.
...
- **Direct fidelity ablations for your exact question are scarce.** No published paper ablates "correct vs. wrong-monitor screenshot" or "synchronized dual-monitor vs. active-monitor-only" for next-action prediction — the multi-monitor case is simply absent from benchmarks. The recommendation to defer dual-monitor fidelity is therefore an inference from absence-of-evidence plus the single-screen design of all major benchmarks, not a direct measurement.
