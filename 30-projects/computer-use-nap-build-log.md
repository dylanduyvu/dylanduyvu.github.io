...
> [!success] Roughly 200-row labeling push complete, 2026-07-29
> The third ten-video batch is fully merged through `compact_monitor_3_1785173529401.mp4`. The canonical corpus now contains 220 retained candidates, including 196 accepted rows. The final seven recordings added 30 accepted events and three non-accepted inferred Gmail-gap candidates. A gesture-mediated Mission Control or App Exposé focus is now explicitly a valid shortcut opportunity at window/file granularity; any later editor click remains a separate event. Stop labeling and freeze the expanded-history experiment before the next model call.

> [!important] V5 expanded-history design approved, 2026-07-29
> Dylan approved a 15-target, 30-call paired experiment using `gpt-5.6-sol` at `max`. State-only receives the current strictly-prior monitor-3 screenshot. Hybrid history receives the identical current screenshot, every earlier accepted history action as structured text, and the ten most recent recoverable earlier screenshot/action pairs. Fifteen targets are selected at fixed midpoint quantiles across the final eligible chronological pool. Exact and semantic top-1/top-3, paired condition outcomes, action type, and blinded shortcut usefulness are reported. V4 remains frozen and unrun. Official design: [[computer-use-nap-v5-expanded-history-experiment-design-2026-07-29|Computer-use NAP V5 expanded-history experiment design, July 29, 2026]].
