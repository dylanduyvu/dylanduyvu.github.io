...
Computer use removes the interface tax between what you want and what the machine does. There is a valuable product to be built here. But, work needs to be done to make it 10x. Still, its value proposition is sound: fewer clicks, less friction. Nobody ever wants more clicks to do what they want on their computer. Jeff Bezos built Amazon on what won't change: nobody will ever want [higher prices or slower delivery](https://quoteinvestigator.com/2021/03/03/not-change/). And fewer clicks is the same kind of bet. We built the tightest version of this design, watched where it broke, and traced the break to two missing pieces. This post is the map.
...
We tried to build it, but the scope ran downward into the stack instead. Section 8 holds what we found there. We never validated the design in ordinary use.
...
Latency is key at every part of the stack for productizing this; it is not the immediate bottleneck to accurate prediction (section 9). So each part becomes a question: does a usable version already exist to adopt? Can this be packaged? If not, what needs to be built?
...
Two takeaways, stated precisely. First: fragility spent the runway, so we never cleanly tested the product idea. We never got to find out. Second: the one clean offline result caps expectations anyway. V5, our strongest offline arm, ran a frontier model at max reasoning with screenshots and full history. It went from 0 out of 10 to 5 out of 10 on exact top-3. But every hit was the recurring Codex task or its composer. Arc browser targets: 0 of 5. And it missed a visible Subscribe button while proposing returns to the loop. History is a habit prior, not an intent signal.
...
**A shared missing primitive.** Automatic action labeling from raw recordings, called inverse dynamics. Labs built it to pseudo-label pretraining corpora. The same capability would also make the personal recorder automatic. But today it ships as [open research code]([LINK: videoagenttrek]), not as a product.
