...
| Tool-call reliability | model Performance tab "Tool Call Error Rate" (source claim: data-source audit); `:exacto` pool membership (model pages name an Exacto routing mode) (direct); model-list `min_tool_success_rate` / `max_tool_success_rate` filters (direct, added in later 2026-08-19 schema check); OpenAPI **Benchmarks** group (direct) | public model filter / scrape / unknown | none | model-level filter; endpoint page | model filter uses tool-call finish-rate and does not identify the qualifying endpoint; page metric tests schema validity; scrape fragility | Medium for aggregate filter; Low per endpoint | Partial |
...
| Geography | **not in endpoints API (negative result, direct schema read)**; public model-list `region=eu|us` filter (direct, added in later 2026-08-19 schema check); provider docs/press; `eu.openrouter.ai` EU-filtered model list (direct: [docs](https://openrouter.ai/docs/api/api-reference/models/list-models-user)) | public model filter / varies | none | model-level eligibility; per provider manual | model filter does not identify endpoint/provider or datacenter; provider-declared detail | Medium for aggregate filter; Low per endpoint | Partial |
...
- 2026-08-19: Later OpenAPI and live model-list checks found aggregate `region=eu|us` and `min_tool_success_rate` / `max_tool_success_rate` filters plus `hugging_face_id` in model records. This partly patches model-level discovery. It does not identify the qualifying endpoint or provider organization, and an OpenRouter Hugging Face ID remains a source claim until the model lab's official publisher is checked.
