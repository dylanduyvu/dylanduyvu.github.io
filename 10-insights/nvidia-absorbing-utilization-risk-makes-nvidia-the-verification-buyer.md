...
- 2026-04-06 (Friedman/Mellor): NVIDIA already operates as the technical gate on the market's biggest deal. Lender funds in the CoreWeave/Meta facility can only be drawn after CoreWeave passes NVIDIA's Level 3 testing (GPUs performing as designed in a stable environment). Before the backstop existed, the vendor was already the draw-gate inspector: the verification seat's embryo, occupied by NVIDIA, on a deal NVIDIA has no guarantee exposure to.
- Boundary sharpened (2026-07-06): the metering need is specific to NVIDIA's floor-and-share structure. AMD's backstop (running since 2025) absorbs the same risk by BECOMING THE TENANT - renting unsold capacity back for internal software development - which requires essentially no third-party settlement measurement. So "vendor backstop" does not automatically create a verification buyer; NVIDIA's financial-guarantee design does.
- Volume argument (2026-07-08): buffered loan structures RELOCATE performance risk rather than destroying it (onto operator equity, customers, and vendor guarantees), so total performance exposure grows with the buildout regardless of how loans are labeled - and the fastest-growing pool of it is NVIDIA's own contingent book ($77.5B modeled for F1/27, ~$175B by F1/29), a pure pay-when-performance-slips position. Verification demand follows the risk, not the loans; even if performance-sensitive LENDING stays small, the performance-sensitive POSITIONS balloon, and NVIDIA holds the largest one.
